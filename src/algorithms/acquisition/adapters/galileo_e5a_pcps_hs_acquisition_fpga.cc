      ...
      acq_iq_(configuration->property(role + ".acquire_iq", false))
{
    ...
    codes_ = volk_gnsssdr::vector<volk_gnsssdr::vector<std::complex<float>>>(GALILEO_E5A_NUMBER_OF_CODES, volk_gnsssdr::vector<std::complex<float>>(fft_size));
    ...
    for (uint32_t PRN = 1; PRN <= GALILEO_E5A_NUMBER_OF_CODES; PRN++)
        {
            galileo_e5_a_code_gen_complex_sampled(code_aux, PRN, signal_, fs_in_, 0, num_codes);
